...
#  ifdef ENABLE_COLORS
#   define STRACE_PRINT_COLOR_SEQ(kind_) do { if (color_is_enabled) STRACE_PRINTS(color_seq(kind_)); } while(0)
#  else
...
tprints_sysret_next(const char *name)
{
	...
	tprint_space();
	if (name) {
		if ((name[0] == 'e' && (!strcmp(name, "error") || !strcmp(name, "errno")))
		    || (name[0] == 's' && !strcmp(name, "strerror"))) {
